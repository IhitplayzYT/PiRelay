...
impl PiRelayClient {
    pub fn new(conn: Connection) -> Self {
        ...
        Self {client: Client::new(),connection:conn,username:uname}
    }
    ...
    pub async fn send_config(&self, config_json: &str) -> Result<(), Box<dyn std::error::Error>> {
        ...
        let response = self.client.post(&url).body(config_json.to_string()).send().await?;
    ...
    pub async fn send_files(&self, send_map: &HashMap<String, Vec<PathBuf>>) -> Result<(), Box<dyn std::error::Error>> {
    ...
    fn send_directory<'a>(&'a self, recipient: &'a str, dir_path: &'a Path, base_dir: &'a Path) -> std::pin::Pin<Box<dyn std::future::Future<Output = Result<(), Box<dyn std::error::Error>>> + 'a>> {
    ...
    async fn send_file(&self, recipient: &str, file_path: &Path, base_dir: &Path) -> Result<(), Box<dyn std::error::Error>> {
        let rel_path = file_path.strip_prefix(base_dir).unwrap_or(file_path).to_str().ok_or("Invalid path")?;
        let file_id = format!("{}_{}", self.username, rel_path.replace("/", "_"));
        let file_size = fs::metadata(file_path)?.len() as usize;
        println!("Sending file: {:?} (Rel Path: {}, Size: {} B)", file_path, rel_path, file_size);
        if file_size <= SMALL_FILE_SIZE {
            self.send_small_file(recipient, &file_id, file_path, rel_path).await?;
        } else {
            self.send_large_file_chunked(recipient, &file_id, file_path, rel_path).await?;
        }

        Ok(())
    }

    async fn send_small_file(&self, recipient: &str, file_id: &str, file_path: &Path, relative_path: &str) -> Result<(), Box<dyn std::error::Error>> {
        ...
        let form = reqwest::multipart::Form::new().part("file", reqwest::multipart::Part::bytes(file_content).file_name(file_name.to_string())).text("file_id", file_id.to_string()).text("sender", self.username.clone()).text("relative_path", relative_path.to_string());
    ...
    async fn send_large_file_chunked(&self, recipient: &str, file_id: &str, file_path: &Path, relative_path: &str) -> Result<(), Box<dyn std::error::Error>> {
        ...
        for (chunk_idx, start) in (0..file_data.len()).step_by(CHUNK_SIZE).enumerate() {
            ...
            let url = format!("{}/send/{}/{}/{}", self.base_url(), recipient, file_id, chunk_idx);
            ...
            if resp.status().is_success() {
                ...
                eprintln!("Failed to send chunk {}/{}: {}", chunk_idx + 1, tot_chunks, resp.status());
    ...
    pub async fn receive_files(&self, senders: &[String], output_dir: Option<&PathBuf>) -> Result<(), Box<dyn std::error::Error>> {
        if senders.is_empty() {
    ...
    async fn receive_from_sender(&self, sender: &str, output_dir: Option<&PathBuf>) -> Result<(), Box<dyn std::error::Error>> {
        let url = if sender.is_empty() {
            format!("{}/recieve/", self.base_url())
        } else {
            format!("{}/recieve/{}", self.base_url(), sender)
        };
        println!("Receiving files from: {}", if sender.is_empty() { "MailBox" } else { sender });
        let resp = self.client.get(&url).header("X-Receiver", &self.username).send().await?;
        if resp.status().is_success() {
            let content_type = resp.headers().get("content-type").and_then(|ct| ct.to_str().ok()).unwrap_or("application/octet-stream");
            let file_name_header = resp.headers().get("X-File-Name").and_then(|f| f.to_str().ok()).map(|s| s.to_string());
            if content_type.contains("multipart") {
                ...
                if let Ok(files_info) = serde_json::from_slice::<Vec<FileInfo>>(&data) {
                    for file_info in files_info {
                        println!("Remote Unsynced: {} (size: {})", file_info.name, file_info.size);
                    }
                } else {
    ...
    async fn handle_multipart_response(&self, response: reqwest::Response, output_dir: Option<&PathBuf>) -> Result<(), Box<dyn std::error::Error>> {
        let content_type = response.headers().get("content-type").and_then(|ct| ct.to_str().ok()).unwrap_or("application/octet-stream");    
        let boundary = content_type.split("boundary=").nth(1).map(|b| b.trim().trim_matches('"')).ok_or("No boundary found in multipart content-type")?;
...
pub async fn receive(clargs: &CLI) -> Result<(), Box<dyn std::error::Error>> {
    let client = PiRelayClient::new(clargs.connection.clone());
    client.receive_files(&clargs.recieve, clargs.srcdir.as_ref()).await?;
    Ok(())
}

pub async fn send_config(clargs: &CLI, config_json: &str) -> Result<(), Box<dyn std::error::Error>> {
    let client = PiRelayClient::new(clargs.connection.clone());
